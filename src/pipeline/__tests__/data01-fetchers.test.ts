...
describe('DATA-01: fetcher module contracts', () => {
  ...
  it('exports 4 individual fetchers (inform, gpi, ucdp, advisories)', async () => {
    const mod = await import('../fetchers/index.js');
    ...
    assert.equal(typeof mod.fetchUcdp, 'function', 'fetchUcdp should be exported');
  ...
  it('fetchAllSources uses Promise.allSettled for fault isolation', async () => {
    ...
    // others still complete. We test this via UCDP which returns failure
    // without network and does not block other module loading.
  ...
  it('sources.json defines 4 data sources with URLs', async () => {
    ...
    assert.ok(keys.includes('ucdp'), 'Should include ucdp source');
