...
export const hubBlurbTemplates: Record<EnrichedHubType, Record<Lang, string>> = {
  ...
  'countries-to-avoid': {
    en: '{name} scores {score}/10 — {n} of {total} advisories are currently at the highest level (Do Not Travel).',
    it: '{name} ottiene {score}/10 — {n} avvisi su {total} sono attualmente al livello massimo (non viaggiare).',
    es: '{name} obtiene {score}/10 — {n} de {total} avisos están actualmente en el nivel máximo (no viajar).',
    fr: '{name} obtient {score}/10 — {n} avis sur {total} sont actuellement au niveau maximal (ne pas voyager).',
    pt: '{name} obtém {score}/10 — {n} de {total} avisos estão atualmente no nível máximo (não viaje).',
    zh: '{name} 得分 {score}/10——{total} 项警告中有 {n} 项目前处于最高级别（切勿前往）。',
    de: '{name} erreicht {score}/10 — {n} von {total} Reisehinweisen liegen derzeit auf der höchsten Stufe (nicht reisen).',
  },
  'improving-safety': {
