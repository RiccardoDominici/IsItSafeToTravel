...
export const aboutCopy: Record<Lang, AboutCopy> = {
  en: {
    ...
    projectText: 'IsItSafeToTravel is a free, independently run travel safety platform that provides composite safety scores for 248 countries worldwide. Scores are recomputed every day from 40+ public sources: travel advisories from 37 governments plus the World Bank, V-Dem Institute, INFORM Risk Index, Global Peace Index, UCDP, ReliefWeb and GDACS.',
    ...
    sourcesText: 'We aggregate data from 40+ public sources updated daily — travel advisories from 37 governments plus global indices. See our {link} for full details on how scores are calculated.',
  ...
  it: {
    ...
    sourcesText: 'Aggreghiamo dati da 40+ fonti pubbliche aggiornate quotidianamente — avvisi di viaggio di 37 governi più indici globali. Consulta la nostra {link} per tutti i dettagli su come vengono calcolati i punteggi.',
  ...
  es: {
    ...
    projectText: 'IsItSafeToTravel es una plataforma de seguridad en viajes gratuita y de gestión independiente que proporciona puntuaciones de seguridad compuestas para 248 países en todo el mundo. Las puntuaciones se recalculan cada día a partir de 40+ fuentes públicas: avisos de viaje de 37 gobiernos, más Banco Mundial, V-Dem Institute, INFORM, Global Peace Index, UCDP, ReliefWeb y GDACS.',
    missionTitle: 'Misión',
    ...
    sourcesText: 'Agregamos datos de 40+ fuentes públicas actualizadas diariamente — avisos de viaje de 37 gobiernos más índices globales. Consulta nuestra {link} para todos los detalles sobre cómo se calculan las puntuaciones.',
  ...
  fr: {
    ...
    projectText: 'IsItSafeToTravel est une plateforme de sécurité des voyages gratuite et gérée de façon indépendante, qui fournit des scores de sécurité composites pour 248 pays dans le monde. Les scores sont recalculés chaque jour à partir de 40+ sources publiques : les avis de voyage de 37 gouvernements, plus la Banque Mondiale, le V-Dem Institute, INFORM, le Global Peace Index, UCDP, ReliefWeb et GDACS.',
    missionTitle: 'Mission',
    ...
    sourcesText: 'Nous agrégeons des données de 40+ sources publiques mises à jour quotidiennement — avis de voyage de 37 gouvernements plus des indices mondiaux. Consultez notre {link} pour tous les détails sur le calcul des scores.',
  ...
  pt: {
    ...
    sourcesText: 'Agregamos dados de 40+ fontes públicas atualizadas diariamente — avisos de viagem de 37 governos mais índices globais. Consulte nossa {link} para todos os detalhes sobre como as pontuações são calculadas.',
  ...
  zh: {
    ...
    projectTitle: '关于本项目',
    projectText: 'IsItSafeToTravel 是一个免费、独立运营的旅行安全平台，为全球 248 个国家提供综合安全评分。评分每日基于 40+ 个公开来源重新计算：来自 37 个政府的旅行建议，以及世界银行、V-Dem Institute、INFORM 风险指数、全球和平指数、UCDP、ReliefWeb 和 GDACS。',
    missionTitle: '使命',
    ...
    sourcesTitle: '数据来源',
    sourcesText: '我们聚合每日更新的 40+ 个公开来源数据——来自 37 个政府的旅行建议及多项全球指数。详见 {link}，了解评分的完整计算方式。',
  ...
  de: {
    ...
    projectText: 'IsItSafeToTravel ist eine kostenlose, unabhängig betriebene Plattform für Reisesicherheit, die zusammengesetzte Sicherheitsbewertungen für 248 Länder weltweit bereitstellt. Die Bewertungen werden täglich aus über 40 öffentlichen Quellen neu berechnet: Reisehinweise von 37 Regierungen sowie Weltbank, V-Dem Institute, INFORM-Risikoindex, Globaler Friedensindex, UCDP, ReliefWeb und GDACS.',
    missionTitle: 'Mission',
    ...
    sourcesText: 'Wir aggregieren Daten aus über 40 täglich aktualisierten öffentlichen Quellen — Reisehinweise von 37 Regierungen sowie globale Indizes. Vollständige Details zur Berechnung der Bewertungen finden Sie auf unserer {link}.',
