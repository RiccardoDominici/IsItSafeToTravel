// Color scale re-exported from the dependency-free colors.ts (this module used
// to import all of d3 statically, pulling ~15 KB of d3-scale/d3-color into
// every client chunk that only wanted a score color).
import { scoreToColor as safetyColorScale } from './colors';

export { safetyColorScale };

// --- ISO 3166-1 numeric to alpha-3 mapping ---
// The world-atlas topojson uses numeric IDs; our ScoredCountry data uses alpha-3.
export const ISO_NUMERIC_TO_ALPHA3: Record<string, string> = {
  ...
  '246': 'FIN', '248': 'ALA', '250': 'FRA', '254': 'GUF', '258': 'PYF',
  ...
  '498': 'MDA', '499': 'MNE', '500': 'MSR', '504': 'MAR', '508': 'MOZ',
...
/**
 * Maps a safety score (1-10) to a CSS color string.
 * 1 = most dangerous (red), 10 = safest (blue).
 * (Implementation lives in colors.ts; re-exported here under the map's name.)
 */

/**
 * Returns the fill color for a country on the map.
 * Scored countries get a gradient color; unscored get neutral gray.
 */
export function getCountryColor(
  ...
  if (score !== undefined) {
    return safetyColorScale(score);
  }
  return isDark ? UNSCORED_COLOR_DARK : UNSCORED_COLOR;
}

/** Processed country data for the map component */
